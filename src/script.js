  ...
  return fetch("https://njkfull.github.io/files.html")
    ...
    .then((data) => {
      ...
      function buildSongElement(index, name) {
        ...
        songElement.setAttribute("class", "play-list-row");
        songElement.setAttribute("data-track-row", index);
        let div1 = document.createElement("div");
        div1.setAttribute("class", "small-toggle-btn");
        let i1 = document.createElement("i");
        i1.setAttribute("class", "small-play-btn");
        let span1 = document.createElement("i");
        span1.setAttribute("class", "screen-reader-text");
        span1.textContent = "Small toggle button";
        div1.appendChild(i1);
        div1.appendChild(span1);
        let div2 = document.createElement("div");
        div2.setAttribute("class", "track-number");
        div2.textContent = index + ".";
        let div3 = document.createElement("div");
        div3.setAttribute("class", "track-title");
        let a1 = document.createElement("a");
        a1.setAttribute("class", "playlist-track");
        a1.setAttribute("href", "#");
        a1.setAttribute("data-play-track", index);
        a1.textContent = name;
        div3.appendChild(a1);
        songElement.appendChild(div1);
        songElement.appendChild(div2);
        songElement.appendChild(div3);

        return songElement;
      }

      for (let i = 1; i <= songs.length; i++) {
        if (!songs[i - 1]) continue;
        let musicLink = getLink(songs[i - 1]);
        let name = getName(musicLink);
        let sourceElnt = document.createElement("source");
        sourceElnt.setAttribute("src", musicLink);
        sourceElnt.setAttribute("data-track-number", i);
        document.getElementById("audio").appendChild(sourceElnt);
        document
          .getElementById("main-list")
          .appendChild(buildSongElement(i, name));
      }
    });
}
...
function loadDataTest() {
  ...
  for (let i = 1; i <= songs.length; i++) {
    ...
    sourceElnt.setAttribute("data-track-number", i);
    document.getElementById("audio").appendChild(sourceElnt);
    document.getElementById("main-list").appendChild(buildSongElement(i, name));
  }
}
